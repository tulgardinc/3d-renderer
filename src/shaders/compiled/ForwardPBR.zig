...
&.{.{ .binding = 0, .visibility = gpu.ShaderStage.vertex | gpu.ShaderStage.fragment, .type = .{ .buffer = .{.type = .uniform, .has_dynamic_offset = false, .min_binding_size = 32 } } },.{ .binding = 1, .visibility = gpu.ShaderStage.vertex | gpu.ShaderStage.fragment, .type = .{ .texture = .{ .sample_type = .float, .view_dimension = .@"2d", .multi_sampled = false  } } },.{ .binding = 2, .visibility = gpu.ShaderStage.vertex | gpu.ShaderStage.fragment, .type = .{ .sampler = .filtering } },},
